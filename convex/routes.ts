...
export const listByOptimization = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query("routes")
      .withIndex("by_optimization_run", (q) => q.eq("optimizationRunId", args.optimizationRunId))
      .collect();
  },
});
...
export const getStatsByOptimization = query({
  ...
  handler: async (ctx, args) => {
    ...
    const stats = {
      ...
      totalWaitingTime: routes.reduce((sum, r) => sum + (r.waitingTime || 0), 0),
      totalServiceTime: routes.reduce((sum, r) => sum + (r.serviceTime || 0), 0),
      totalSetupTime: routes.reduce((sum, r) => sum + (r.setupTime || 0), 0),
      totalDeliveries: routes.reduce((sum, r) => sum + (r.deliveryCount || 0), 0),
      routesWithViolations: routes.filter(r => r.violations && r.violations.length > 0).length,
      averageCostPerRoute: routes.length > 0 ? routes.reduce((sum, r) => sum + (r.cost || 0), 0) / routes.length : 0,
      averageDistancePerRoute: routes.length > 0 ? routes.reduce((sum, r) => sum + (r.distance || 0), 0) / routes.length : 0,
    };
...
export const listWithFilters = query({
  ...
  handler: async (ctx, args) => {
    ...
    const query = ctx.db.query("routes").withIndex("by_project", (q) => q.eq("projectId", args.projectId));
    ...
    if (args.optimizationRunId) {
      filteredRoutes = filteredRoutes.filter(r => r.optimizationRunId === args.optimizationRunId);
    }
    
    if (args.vehicleId) {
      filteredRoutes = filteredRoutes.filter(r => r.vehicleId === args.vehicleId);
    }
    ...
    if (args.minCost !== undefined) {
      filteredRoutes = filteredRoutes.filter(r => (r.cost || 0) >= args.minCost!);
    }
    
    if (args.maxCost !== undefined) {
      filteredRoutes = filteredRoutes.filter(r => (r.cost || 0) <= args.maxCost!);
    }
    
    if (args.minDistance !== undefined) {
      filteredRoutes = filteredRoutes.filter(r => (r.distance || 0) >= args.minDistance!);
    }
    
    if (args.maxDistance !== undefined) {
      filteredRoutes = filteredRoutes.filter(r => (r.distance || 0) <= args.maxDistance!);
    }
...
export const getEfficiencyMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const totalServiceTime = routes.reduce((sum, r) => sum + (r.serviceTime || 0), 0);
    const totalWaitingTime = routes.reduce((sum, r) => sum + (r.waitingTime || 0), 0);
    const totalDistance = routes.reduce((sum, r) => sum + (r.distance || 0), 0);
    
    // Calculate efficiency metrics
    const timeUtilization = totalDuration > 0 ? (totalServiceTime / totalDuration) * 100 : 0;
