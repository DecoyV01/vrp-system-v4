...
export const projectCreateSchema = z.object({
  ...
  geographicRegion: z
    .string()
    .max(100, 'Geographic region too long')
    .optional(),
  priority: z.enum(['low', 'medium', 'high', 'critical']).optional(),
  estimatedVehicles: z
    .number()
    .int()
    .min(0, 'Estimated vehicles must be non-negative')
    .optional(),
  estimatedJobs: z
    .number()
    .int()
    .min(0, 'Estimated jobs must be non-negative')
    .optional(),
  planningHorizonDays: z
    .number()
    .int()
    .min(1, 'Planning horizon must be at least 1 day')
    .max(365, 'Planning horizon too long')
    .optional(),
...
export const vehicleCreateSchema = z
  .object({
    ...
    profile: z
      .enum(['car', 'truck', 'van', 'bike', 'walking', 'motorcycle'])
      .optional(),
    startLon: z
      .number()
    ...
    endLat: z
      ...
      .optional(),
    capacity: z
      .array(z.number().min(0, 'Capacity must be non-negative'))
      .max(10, 'Too many capacity dimensions')
      .optional(),
    skills: z
      .array(z.number().int().min(0, 'Skill ID must be non-negative'))
      .max(50, 'Too many skills')
      .optional(),
    twStart: z
      .number()
      .int()
      .min(0, 'Time window start must be non-negative')
      .optional(),
    twEnd: z
      .number()
      .int()
      .min(0, 'Time window end must be non-negative')
      .optional(),
    speedFactor: z
      .number()
      .min(0.1, 'Speed factor too low')
      .max(10, 'Speed factor too high')
      .optional(),
    maxTasks: z
      .number()
      .int()
      .min(1, 'Max tasks must be at least 1')
      .max(1000, 'Max tasks too high')
      .optional(),
    maxTravelTime: z
      .number()
      .int()
      .min(0, 'Max travel time must be non-negative')
      .optional(),
    maxDistance: z
      .number()
      .min(0, 'Max distance must be non-negative')
      .optional(),
    costFixed: z.number().min(0, 'Fixed cost must be non-negative').optional(),
    costPerHour: z
      .number()
      .min(0, 'Hourly cost must be non-negative')
      .optional(),
    ...
    datasetVersion: z
      .number()
      .int()
      .min(1, 'Dataset version must be at least 1')
      .optional(),
  })
  .refine(
    data => {
...
export const jobCreateSchema = z
  .object({
    ...
    service: z
      .number()
      .int()
      .min(0, 'Service time must be non-negative')
      .optional(),
    delivery: z
      .array(z.number().min(0, 'Delivery quantity must be non-negative'))
      .max(10, 'Too many delivery dimensions')
      .optional(),
    pickup: z
      .array(z.number().min(0, 'Pickup quantity must be non-negative'))
      .max(10, 'Too many pickup dimensions')
      .optional(),
    skills: z
      .array(z.number().int().min(0, 'Skill ID must be non-negative'))
      .max(50, 'Too many skills')
...
export const routeCreateSchema = z.object({
  ...
  waitingTime: z
    .number()
    .int()
    .min(0, 'Waiting time must be non-negative')
    .optional(),
  serviceTime: z
    .number()
    .int()
  ...
  deliveryCount: z
    .number()
    .int()
    .min(0, 'Delivery count must be non-negative')
    .optional(),
