...
export const create = mutation({
  args: {
    ...
    timeWindows: v.optional(v.array(v.array(v.number()))),
  ...
  handler: async (ctx, args) => {
    ...
    // Validate delivery and pickup arrays (must be exactly 3 elements)
    if (args.delivery !== undefined) {
      validateCapacity(args.delivery, 'delivery')
    }
    if (args.pickup !== undefined) {
      validateCapacity(args.pickup, 'pickup')
    }

    // Validate time unit consistency (all times in seconds)
    validateTimeConsistency(args, 'job')
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Validate delivery and pickup arrays (must be exactly 3 elements)
    if (cleanUpdateData.delivery !== undefined) {
      validateCapacity(cleanUpdateData.delivery as number[], 'delivery')
    }
    if (cleanUpdateData.pickup !== undefined) {
      validateCapacity(cleanUpdateData.pickup as number[], 'pickup')
    }

    // Validate time unit consistency (all times in seconds)
    validateTimeConsistency(cleanUpdateData, 'job')
...
export const bulkImport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const jobData of args.jobs) {
      // Validate delivery and pickup arrays (must be exactly 3 elements)
      if (jobData.delivery !== undefined) {
        validateCapacity(jobData.delivery, 'delivery')
      }
      if (jobData.pickup !== undefined) {
        validateCapacity(jobData.pickup, 'pickup')
      }

      // Validate time unit consistency (all times in seconds)
      validateTimeConsistency(jobData, 'job')
  ...
  handler: async (ctx, args) => {
    ...
    if (args.hasTimeWindows !== undefined) {
      filteredJobs = filteredJobs.filter(j =>
        args.hasTimeWindows
          ? j.timeWindows && j.timeWindows.length > 0
          : !j.timeWindows || j.timeWindows.length === 0
      )
    }
    ...
    if (args.hasDelivery !== undefined) {
      filteredJobs = filteredJobs.filter(j =>
        args.hasDelivery
          ? j.delivery && j.delivery.length > 0
          : !j.delivery || j.delivery.length === 0
      )
    }
...
export const getStatsByDataset = query({
  ...
  handler: async (ctx, args) => {
    ...
    const stats = {
      ...
      jobsWithTimeWindows: jobs.filter(
        j => j.timeWindows && j.timeWindows.length > 0
      ).length,
      jobsWithDelivery: jobs.filter(j => j.delivery && j.delivery.length > 0)
        .length,
      ...
      averagePriority:
        jobs.filter(j => j.priority !== undefined).length > 0
          ? jobs
              .filter(j => j.priority !== undefined)
              .reduce((sum, j) => sum + j.priority!, 0) /
            jobs.filter(j => j.priority !== undefined).length
          : 0,
      totalDeliveryQuantity: jobs.reduce(
        (sum, j) => sum + (j.delivery?.reduce((s, d) => s + d, 0) || 0),
        0
      ),
      totalPickupQuantity: jobs.reduce(
        (sum, j) => sum + (j.pickup?.reduce((s, p) => s + p, 0) || 0),
        0
      ),
    }
