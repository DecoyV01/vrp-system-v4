...
export const useHierarchyOperations = (): UseHierarchyOperationsReturn => {
    ...
    async (
      ...
      try {
        ...
        switch (entityType) {
          case 'project': {
            ...
            try {
              ...
              cascadeInfo.warnings = [
                'Deleting this project will delete all its scenarios and datasets',
              ]
            }
            break
          }
          ...
          case 'scenario': {
            try {
              ...
              cascadeInfo.warnings = [
                'Deleting this scenario will delete all its datasets',
              ]
            }
            break
          }
          ...
          case 'dataset': {
            try {
              ...
              cascadeInfo.warnings = [
                'Deleting this dataset will delete all its data',
              ]
            }
            break
          }
        ...
        return {
          ...
          childType: '',
          warnings: [
            `Deleting this ${entityType} may affect related data. Please proceed with caution.`,
          ],
          affectedEntities: {},
        }
      }
      ...
      try {
        ...
        if (!options.skipCascadeCheck) {
          ...
          if (cascadeInfo.childCount > 0) {
            console.log(
              `Performing cascading delete: ${cascadeInfo.childCount} related entities will be deleted`
            )
          }
        }
    ...
    async (
      ...
      try {
        ...
        switch (entityType) {
          ...
          default:
            throw new Error(
              `Clone not supported for entity type: ${entityType}`
            )
        }
  ...
  const bulkDelete = useCallback(
    async (
      ...
      try {
        ...
        if (success > 0) {
          console.log(
            `Bulk delete completed: ${success} ${entityType}${success !== 1 ? 's' : ''} deleted successfully`
          )
        }

        if (failed > 0) {
          console.error(
            `Bulk delete errors: ${failed} ${entityType}${failed !== 1 ? 's' : ''} failed to delete`
          )
  ...
  const bulkClone = useCallback(
    async (
      ...
      try {
        ...
        for (let i = 0; i < sourceIds.length; i++) {
          try {
            ...
            const message =
              error instanceof Error ? error.message : 'Unknown error'
            errors.push(
              `Failed to clone ${entityType} ${sourceIds[i]}: ${message}`
            )
          }
        }
        ...
        if (success > 0) {
          console.log(
            `Successfully cloned ${success} ${entityType}${success !== 1 ? 's' : ''}`
          )
        }

        if (failed > 0) {
          console.log(
            `Failed to clone ${failed} ${entityType}${failed !== 1 ? 's' : ''}`
          )
        }
  ...
  const validateEntityName = useCallback(
    async (
      name: string,
