...
const MAPBOX_TOKEN = import.meta.env.VITE_MAPBOX_TOKEN || 'pk.eyJ1IjoidnJwLXN5c3RlbSIsImEiOiJjbHNuNXBhbmIwMDA0M2twbTZmYTBlcnE3In0.example'
...
export const LocationMap = ({
  ...
  }, [viewport.latitude, viewport.longitude, viewport.zoom, interactive, onViewportChange, onLocationCreate, isCreatingLocation])
  ...
  useEffect(() => {
    ...
    locations.forEach(location => {
      ...
      el.style.backgroundColor = typeColors[location.locationType || 'customer'] || '#6B7280'
    ...
    if (locations.length > 0) {
      const validLocations = locations.filter(l => l.locationLat && l.locationLon)
  ...
  const handleToggleCreateMode = useCallback(() => {
    ...
    if (mapRef.current) {
      mapRef.current.getCanvas().style.cursor = isCreatingLocation ? '' : 'crosshair'
    }
  }, [isCreatingLocation])
  ...
  if (!MAPBOX_TOKEN || MAPBOX_TOKEN.includes('example')) {
    return (
      <div className={`h-full bg-gray-100 flex items-center justify-center ${className}`}>
                ...
                {locations.map(location => (
                  <div
                    ...
                    className={`p-2 border rounded cursor-pointer text-left ${
                      location._id === selectedLocationId ? 'bg-blue-50 border-blue-200' : 'hover:bg-gray-50'
                    }`}
    ...
    <div className={`relative h-full ${className}`}>
      ...
      {isCreatingLocation && (
        <div className="absolute top-4 right-4 bg-blue-600 text-white px-3 py-2 rounded-lg shadow-lg">
          <div className="text-sm font-medium">Click on map to create location</div>
          <div className="text-xs opacity-90">Click anywhere to place a new location</div>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4">
        <Badge variant="secondary" className="bg-white/90 backdrop-blur-sm">
          {locations.filter(l => l.locationLat && l.locationLon).length} of {locations.length} locations on map
        </Badge>
      </div>
    </div>
  )
