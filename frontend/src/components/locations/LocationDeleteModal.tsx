...
export const LocationDeleteModal = ({
  ...
  const analyzeLocationUsage = async (
    locationId: string
    ...
    if (mockUsage.vehicleCount > 0) {
      mockUsage.warnings.push(
        `${mockUsage.vehicleCount} vehicle${mockUsage.vehicleCount !== 1 ? 's' : ''} reference this location`
      )
    ...
    if (mockUsage.jobCount > 0) {
      mockUsage.warnings.push(
        `${mockUsage.jobCount} job${mockUsage.jobCount !== 1 ? 's' : ''} reference this location`
      )
      mockUsage.canDelete = false
    }
    ...
    if (mockUsage.shipmentCount > 0) {
      mockUsage.warnings.push(
        `${mockUsage.shipmentCount} shipment${mockUsage.shipmentCount !== 1 ? 's' : ''} reference this location`
      )
      mockUsage.canDelete = false
    }
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="text-center p-2 bg-muted rounded">
                  <Truck className="w-4 h-4 text-foreground mx-auto mb-1" />
                  <div className="text-sm font-medium">
                    {usageAnalysis.vehicleCount}
                  </div>
                ...
                <div className="text-center p-2 bg-green-50 rounded">
                  ...
                  <div className="text-sm font-medium">
                    {usageAnalysis.jobCount}
                  </div>
                  <div className="text-xs text-gray-600">Jobs</div>
                </div>
                <div className="text-center p-2 bg-muted rounded">
                  <Route className="w-4 h-4 text-foreground mx-auto mb-1" />
                  <div className="text-sm font-medium">
                    {usageAnalysis.shipmentCount}
                  </div>
              ...
              {usageAnalysis.warnings.length > 0 && (
                <Alert
                  variant={usageAnalysis.canDelete ? 'default' : 'destructive'}
                >
                  ...
                  <AlertDescription>
                    ...
                    {!usageAnalysis.canDelete && (
                      <div className="mt-2 text-sm font-medium">
                        You must remove these references before deleting the
                        location.
                      </div>
                    )}
                  </AlertDescription>
              ...
              {usageAnalysis.canDelete &&
                usageAnalysis.warnings.length === 0 && (
                  <Alert>
                    <Info className="h-4 w-4" />
                    <AlertDescription>
                      This location is not referenced by any vehicles, jobs, or
                      shipments. It's safe to delete.
                    </AlertDescription>
                  </Alert>
                )}
            </div>
